...
survival[,1] <- paste0(survival[,1],"-01")
...
for(i in 1:length(out$factorizations)){
  ...
  pvalues <- length(out$factorizations)*as.matrix(coef(summary(coxph_obj))[,5])
...
png(file=paste0(results_folder, "survival_", cancer, ".png"), width = 15, height = 15, units = 'in', res = 200)
matplot(1:length(out$method), t(surv_final), 
        col="black", pch=18, xlab="Method", ylab="Pvalues survival", xaxt="none", cex=1.5)
abline(h = (-log10(0.05)), v=0, col="black", lty=3, lwd=3)
axis(1, at=1:length(out$method), labels=colnames(surv_final)) 
dev.off()

...
me <- read.csv('ME_train.csv', header = TRUE, row.names = 1)
...
write.csv(factors_mcia, paste0(savedirectory, 'geme_train_factors_', nf, '.csv'), quote=F, row.names=T)

projection <- as.matrix(factorizations_mcia$mcoa$axis)

write.csv(projection, paste0(savedirectory, 'geme_train_projection_', nf, '.csv'), quote=F, row.names=T)

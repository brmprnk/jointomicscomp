...
from src.nets import MLP, CrossGeneratingVariationalAutoencoder
...
encoder_layers = [int(kk) for kk in '256-128-64'.split('-')]
decoder_layers = encoder_layers[::-1][1:]

# load pre-trained MoE model
model = CrossGeneratingVariationalAutoencoder(input_dims, encoder_layers, decoder_layers,
 likelihoods, True, 0.0, 'Adam', 0.0001, 0.0001, 'normal', 1.0, 1.0, 1.0, [True, True])
...
checkpoint = torch.load('results/tmpnbcite/results/likelihood-cite-cgae-49_RNA_ADT/CGAE/checkpoint/model_best.pth.tar')
...
logger.success("Loaded trained CGVAE model.")

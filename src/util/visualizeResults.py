...
modelNames = ['baseline', 'MOFA+', 'CGVAE', 'ccVAE', 'PoE', 'MoE']
...
for oo, omic in enumerate(['omic1+2', 'omic1', 'omic2']):
    ...
    for jjj in range(f1.shape[1]):
        for clsi in range(f1.shape[2]):
            names.append(models[jjj] + '_' + omicNames[oo])
...
sns.heatmap(svmf1, square=True, vmin=0, vmax=1, cbar=True, ax=ax, xticklabels=labelNames)
ax.tick_params(axis='x', which='major', labelsize=5)
ax.set_yticks([3*i+1.5 for i in range(6)])
ax.set_yticklabels(modelNames)

for i in range(1,6):
    ax.axhline(3 * i, color='b')

ax.set_title('SVM')

ax = fig.add_subplot(2,1,2)

sns.heatmap(mlpf1, square=True, vmin=0, vmax=1, cbar=True, ax=ax, xticklabels=labelNames)
ax.tick_params(axis='x', which='major', labelsize=5)
ax.set_yticks([3*i+1.5 for i in range(6)])
ax.set_yticklabels(modelNames)
...
# fig.savefig('figures/mlp_performance_per_class.eps', dpi=600)

...
class SingleOmicsDataset:
    def __init__(self, omic1, omic_labels):
        # load the two data matrices
        self.d1 = torch.tensor(omic1)
        self.y = torch.tensor(omic_labels)
...
class MultiOmicsDataset(SingleOmicsDataset):
    def __init__(self, omic1, omic2, data_labels):
        # constructor of parent class takes care of first dataset & labels
        super(MultiOmicsDataset, self).__init__(omic1, data_labels)

        # load the 2nd data view
        self.d2 = torch.tensor(omic2)
